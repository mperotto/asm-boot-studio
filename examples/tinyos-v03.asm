; ================================================================
; TinyOS v0.3 — 8 setores (4096 bytes)
; PM32 | BIOS 8x8 font | Terminal 40x22 | Teclado | PS/2 Mouse
;
; Teclas: imprimíveis → terminal | Enter | Backspace | ESC=cor
; Mouse: move cursor vermelho
; ================================================================
...
SECTION boot vstart=0x7C00
    ...
    ; VGA modo 13h: 320x200, 256 cores
    mov ax, 0x0013
    int 0x10

    ; Font embutida no kernel — nenhuma leitura de BIOS necessária
...
; ---- Entrada do kernel ----
    ...
    mov ss, ax
    mov esp, 0x9F000
    cld                     ; direction flag = forward (req para rep movsd/stosd)
...
str_title:   db " TinyOS v0.3 | PM32 | KB+Mouse | VGA 320x200 ", 0
str_welcome: db " Type=print  Enter=newline  BS=del  ESC=color", 0
...
render_str:
    ...
    push ecx
.rs_next:
    cmp ebx, W          ; para ao atingir borda direita (col 40)
    jge .rs_done
    lodsb
    test al, al
    jz .rs_done
...
.bs:
    cmp word [cur_col], 0
    jne .bs_same
    ; col=0: wrap to previous row
    cmp word [cur_row], 2       ; row 2 = first user row
    je .done                    ; already at top-left, nothing to do
    dec word [cur_row]
...
scroll_up:
    ...
    push ecx
    push eax
    mov esi, VGA + TEXT_Y1*W + 8*W
    mov edi, VGA + TEXT_Y1*W
    mov ecx, (TEXT_Y2 - TEXT_Y1 - 8)*W / 4
    rep movsd
    mov edi, VGA + (TEXT_Y2 - 8)*W
    mov ecx, 8*W / 4
    mov eax, (BG<<24)|(BG<<16)|(BG<<8)|BG
    rep stosd
    pop eax
